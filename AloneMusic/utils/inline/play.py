...
# Pyrogram sürüm hatasını gidermek için ButtonStyle sınıfını manuel ekliyoruz.
# Bu sayede 'ImportError: cannot import name ButtonStyle' hatası almazsın.
class ButtonStyle:
    PRIMARY = 1   # Mavi renk
    SECONDARY = 2 # Gri renk
    SUCCESS = 3   # Yeşil renk
    DANGER = 4    # Kırmızı renk
...
def stream_markup_timer(_, chat_id, played, dur):
    ...
    buttons = [
        ...
        [
            InlineKeyboardButton(text="⏭️", callback_data=f"ADMIN Skip|{chat_id}", style=ButtonStyle.PRIMARY),
            InlineKeyboardButton(text=_["CLOSE_BUTTON"], callback_data="close", style=ButtonStyle.DANGER),
            InlineKeyboardButton(text="⏹️", callback_data=f"ADMIN Stop|{chat_id}", style=ButtonStyle.DANGER),
        ],
    ]
    return buttons
